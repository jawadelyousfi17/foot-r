...
export default async function DashboardPage() {
  ...
  const [teams, competitions] = await Promise.all([
    ...
    session.user.isAdmin
      ? prisma.competition.findMany({ where: { ownerId: session.user.id }, orderBy: { createdAt: "desc" }, include: { _count: { select: { groups: true, matches: true } } } })
      : Promise.resolve([]),
  ]);
  ...
  return <main className="mx-auto max-w-7xl px-5 py-12 lg:px-8">
    ...
    <div className={`mt-12 grid gap-8 ${session.user.isAdmin ? "lg:grid-cols-2" : ""}`}>
      ...
      {session.user.isAdmin && <section><h2 className="mb-5 text-2xl font-black">Your competitions</h2><div className="space-y-3">{competitions.map(c => <div key={c.id} className="rounded-2xl border border-black/10 bg-white p-5"><div className="flex justify-between"><b>{c.name}</b><span className="text-xs font-bold text-black/40">{c.status}</span></div><p className="mt-2 text-sm text-black/45">{c._count.groups} groups · {c._count.matches} matches</p><div className="mt-4 flex gap-2"><Link href={`/dashboard/competitions/${c.id}`} className="rounded-full bg-black px-4 py-2 text-xs font-bold text-white">Manage</Link><Link href={`/competitions/${c.slug}`} className="rounded-full border border-black/15 px-4 py-2 text-xs font-bold">View</Link>{c.status === "DRAFT" && <form action={publishCompetitionAction.bind(null, c.id)}><button className="rounded-full bg-[#d7ff3f] px-4 py-2 text-xs font-bold">Publish</button></form>}</div></div>)}{!competitions.length && <Empty />}</div></section>}
    </div>
  </main>;
}
