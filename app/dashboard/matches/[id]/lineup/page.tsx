...
export default async function MatchLineupPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const match = await prisma.match.findFirst({ where: { id }, include: { competition: true, lineupEntries: true, homeTeam: { include: { players: { orderBy: { shirtNumber: "asc" } } } }, awayTeam: { include: { players: { orderBy: { shirtNumber: "asc" } } } } } });
  if (!match) notFound();
