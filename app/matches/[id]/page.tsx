...
import { FollowButton, MatchCountdown, MatchTabs, WhoWillWin } from "@/components/match-hero";
...
export default async function MatchPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const statistics = (
    <div>
      <h3 className="mb-4 font-bold">Match statistics</h3>
      <div className="space-y-3">
        ...
        <Stat label="Passes" home={total(match.homeTeamId, "passes")} away={total(match.awayTeamId, "passes")} />
        <Stat label="Correct passes" home={total(match.homeTeamId, "accuratePasses")} away={total(match.awayTeamId, "accuratePasses")} />
    ...
    <main className="min-h-screen bg-black px-3 py-4 text-white sm:px-5 sm:py-6">
      <div className="mx-auto grid max-w-6xl gap-5 lg:grid-cols-[1fr_20rem]">
        <div className="min-w-0 space-y-5">
          {/* Hero banner */}
          <section className="overflow-hidden rounded-[28px] border-2 border-[#272727] bg-[#1b1b1b]">
            {/* Gradient top */}
            <div
              className="relative border-b-2 border-[#3a3a3a] px-4 pb-6 pt-5 sm:px-10"
              style={{
                backgroundImage:
            ...
            >
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3 sm:gap-5">
                  <Link href={`/competitions/${match.competition.slug}`} aria-label="Back" className="grid size-10 shrink-0 place-items-center rounded-full border-2 border-[#3d3d3f] bg-[#1c1c1d] text-white transition hover:bg-black/40 sm:size-12">
            ...
            <div className="px-4 py-8 sm:px-8 sm:py-12">
              <div className="mx-auto grid max-w-3xl grid-cols-[1fr_auto_1fr] items-center gap-3 sm:gap-8">
                ...
                <div className="text-center">
                  {played ? (
                    <>
                      <p className="text-4xl font-black tracking-wide tabular-nums sm:text-6xl">{match.result?.homeScore}<span className="mx-2 text-white/40">-</span>{match.result?.awayScore}</p>
                      {extraLabel && <p className="mt-3 text-sm font-semibold text-[#9f9fa2] sm:text-lg">{extraLabel}</p>}
                    </>
                  ) : (
                    <>
                      <p className="text-3xl font-black tracking-tight sm:text-5xl">{kickoffTime}</p>
            ...
            <div className="border-t-2 border-[#272727] px-3 pb-5 pt-1 sm:px-6">
              <MatchTabs
                tabs={[
                  {
                    label: "Lineups",
...
function HeroTeam({ team, align }: { team: NonNullable<TeamWithPlayers>; align: "left" | "right" }) {
  return (
    <Link href={`/teams/${team.slug}`} className={`group flex min-w-0 items-center gap-3 sm:gap-6 ${align === "right" ? "flex-col sm:flex-row-reverse sm:justify-start sm:text-right" : "flex-col sm:flex-row sm:justify-start sm:text-left"}`}>
      <TeamLogo team={team} />
      <div className="min-w-0 text-center sm:text-inherit">
        <h1 className="truncate text-lg font-normal leading-tight tracking-tight group-hover:underline sm:text-3xl">{team.name}</h1>
        <p className="mt-1.5 text-xs text-[#aaa] sm:text-lg">{team.shortName || "Football club"}</p>
      </div>
    </Link>
  );
}
...
function TeamLogo({ team }: { team: NonNullable<TeamWithPlayers> }) {
  return team.logoUrl ? <span className="block size-16 shrink-0 rounded-full bg-white bg-cover bg-center bg-no-repeat ring-2 ring-white/15 md:size-20" style={{ backgroundImage: `url(${team.logoUrl})` }} /> : <span className="grid size-16 shrink-0 place-items-center rounded-full bg-white/10 text-xl font-black ring-2 ring-white/15 md:size-20">{team.shortName?.slice(0, 3) || team.name.slice(0, 2).toUpperCase()}</span>;
}
...
function MatchLineups({ matchId, homeTeam, awayTeam, ratings, contributions, lineupEntries }: { matchId: string; homeTeam: NonNullable<TeamWithPlayers>; awayTeam: NonNullable<TeamWithPlayers>; ratings: Map<string, number>; contributions: Map<string, Contribution>; lineupEntries: Array<{ playerId: string; teamId: string; role: LineupRole; position: number | null }> }) {
  ...
  const average = (players: Player[]) => players.length ? players.reduce((sum, player) => sum + (ratings.get(player.id) ?? 6), 0) / players.length : 6;

  return <div className="space-y-6">
    <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 rounded-xl bg-[#292929] px-4 py-4 text-sm"><div className="flex items-center gap-3"><span className={`rounded-full px-2.5 py-1 text-xs font-black ${ratingColor(average(home.starters))}`}>{average(home.starters).toFixed(1)}</span><TeamMini team={homeTeam} /><b className="hidden text-white/60 sm:block">1-2-2</b></div><span className="text-xs font-black text-white/30">5 v 5</span><div className="flex flex-row-reverse items-center gap-3"><span className={`rounded-full px-2.5 py-1 text-xs font-black ${ratingColor(average(away.starters))}`}>{average(away.starters).toFixed(1)}</span><TeamMini team={awayTeam} away /><b className="hidden text-white/60 sm:block">1-2-2</b></div></div>
    <div className="relative mx-auto aspect-video w-full max-w-2xl overflow-hidden rounded-xl bg-[#272727] shadow-inner">
      <div className="absolute inset-3 border-2 border-[#353535]" />
      <div className="absolute inset-y-3 left-1/2 border-l-2 border-[#353535]" />
      <div className="absolute left-1/2 top-1/2 size-32 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-[#353535]" />
      <div className="absolute left-1/2 top-1/2 size-2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#3b3b3b]" />
      <div className="absolute left-3 top-1/2 h-[50%] w-[13%] -translate-y-1/2 border-y-2 border-r-2 border-[#353535]" />
      <div className="absolute right-3 top-1/2 h-[50%] w-[13%] -translate-y-1/2 border-y-2 border-l-2 border-[#353535]" />
      <div className="absolute -left-3 top-1/2 h-[25%] w-[8%] -translate-y-1/2 rounded-r-lg border-2 border-[#353535]" />
      <div className="absolute -right-3 top-1/2 h-[25%] w-[8%] -translate-y-1/2 rounded-l-lg border-2 border-[#353535]" />
      {home.starters.map((player, index) => <PitchPlayer key={player.id} player={player} contribution={contributions.get(player.id)} rating={ratings.get(player.id)} x={homePositions[index][0]} y={homePositions[index][1]} color="bg-[#d7ff3f] text-black" />)}
      {away.starters.map((player, index) => <PitchPlayer key={player.id} player={player} contribution={contributions.get(player.id)} rating={ratings.get(player.id)} x={awayPositions[index][0]} y={awayPositions[index][1]} color="bg-black text-white" />)}
    </div>
...
function PitchPlayer({ player, rating, contribution, x, y, color }: { player: Player; rating?: number; contribution?: Contribution; x: number; y: number; color: string }) {
  const name = player.displayName || player.lastName || player.firstName;
  return <div className="absolute z-10 flex w-28 -translate-x-1/2 -translate-y-1/2 flex-col items-center" style={{ left: `${x}%`, top: `${y}%` }}><span className="relative"><span className={`relative grid size-12 place-items-center overflow-hidden rounded-full bg-[#454545] shadow-lg md:size-14 ${color}`}>{player.imageUrl ? <span className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${player.imageUrl})` }} /> : <b>{player.shirtNumber ?? "—"}</b>}</span>{rating !== undefined && <span className={`absolute -right-4 -top-1 rounded-full px-2 py-0.5 text-[10px] font-black shadow ${ratingColor(rating)}`}>{rating.toFixed(1)}</span>}</span><span className="mt-1.5 max-w-full truncate px-1 text-center text-[10px] font-semibold text-white md:text-xs"><span className="mr-1 text-white/45">{player.shirtNumber ?? ""}</span>{name}</span><Contributions value={contribution} /></div>;
}

function TeamMini({ team, away }: { team: NonNullable<TeamWithPlayers>; away?: boolean }) {
  return <div className={`flex min-w-0 items-center gap-2 ${away ? "flex-row-reverse text-right" : ""}`}><span className="size-8 shrink-0 rounded-full bg-white bg-contain bg-center bg-no-repeat" style={team.logoUrl ? { backgroundImage: `url(${team.logoUrl})` } : undefined} /><span className="truncate">{team.shortName || team.name}</span></div>;
}
...
function Stat({ label, home, away }: { label: string; home: string | number; away: string | number }) {
  return <div className="border-b border-white/5 pb-3 last:border-0 last:pb-0"><div className="grid grid-cols-[2rem_1fr_2rem] items-center text-sm"><b className="text-center">{home}</b><span className="text-center text-xs text-white/45">{label}</span><b className="text-center">{away}</b></div></div>;
}
