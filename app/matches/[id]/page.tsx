...
function PitchPlayer({ player, rating, contribution, mvp, x, y }: { player: Player; rating?: number; contribution?: Contribution; mvp?: boolean; x: number; y: number }) {
  ...
  return <div className="absolute z-10 flex w-28 -translate-x-1/2 -translate-y-1/2 flex-col items-center" style={{ left: `${x}%`, top: `${y}%` }}><span className="relative"><span className={`relative grid size-11 place-items-center overflow-hidden rounded-full bg-[#4c4c4c] shadow-md md:size-[3.75rem] ${mvp ? "ring-2 ring-[#d7ff3f]" : ""}`}>{player.imageUrl ? <span className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${player.imageUrl})` }} /> : player.shirtNumber !== null ? <b className="text-white">{player.shirtNumber}</b> : <Icon name="player" size={22} className="text-white/50" />}</span>{mvp && <span className="absolute -left-2 -top-1.5 grid size-5 place-items-center rounded-full bg-[#d7ff3f] text-[10px] font-black text-black shadow" title="Man of the match">★</span>}{rating !== undefined && <span className={`absolute -right-3 -top-1.5 grid h-[1.35rem] min-w-[2.4rem] place-items-center rounded-full px-1.5 text-[11px] font-black shadow ${ratingColor(rating)}`}>{rating.toFixed(1)}</span>}</span><span className="mt-2 max-w-full truncate px-1 text-center text-[11px] font-normal text-[#f3f3f3] md:text-xs"><span className="mr-1 text-[#b4b4b4]">{player.shirtNumber ?? ""}</span>{name}</span><Contributions value={contribution} /></div>;
}

function TeamMini({ team, away }: { team: NonNullable<TeamWithPlayers>; away?: boolean }) {
  return <div className={`flex min-w-0 items-center gap-2 ${away ? "flex-row-reverse text-right" : ""}`}>{team.logoUrl ? <span className="size-8 shrink-0 rounded-full bg-white bg-contain bg-center bg-no-repeat" style={{ backgroundImage: `url(${team.logoUrl})` }} /> : <span className="grid size-8 shrink-0 place-items-center rounded-full bg-white/10 text-white/45"><Icon name="shield" size={15} /></span>}<span className="truncate">{team.name}</span></div>;
}
